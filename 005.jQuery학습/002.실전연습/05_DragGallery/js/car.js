...
// 0-2. 이미지 셋업 : ./360view/coutry1.jpg 형식
// 이미지 개수 : 총 50개
for(let i = 1; i <= 50; i++) {
    cbx.append(`
...
// 자동차박스.찾아('img').숨겨().첫번째().보여() // eq(30) - 30번째
cbx.find('img').hide().first().show();

// 1. 변수 셋팅하기 //////////
// (1) 드래그 상태변수 : 0 - 드래그 아님, 1 - 드래그중
let drag = 0;

// (2) 클릭 시 위치변수(드래그 시작점 - 실제 할당값)
let point = 0;
...
cbx.on('mousemove touchmove', e => {
    ...
    // 타임아웃 시간에 따라 이벤트 수를 조절할 수 있다!

    // 1. x축 위치값
    let pos = e.pageX || e.changedTouches[0].pageX;
    // e.changedTouches[0].pageX - 모바ㅑ일용 x축 위치값
...
// 마우스가 나갈 때도 해제처리해야 드래그하다가 나갈 때 괜찮음
...
const rotateCar = dir => { //  dir방향
    ...
    // (다른방법: ) 위에서 증감 전 숨기기 안하고 아래서 하기
    // 1번 주석 후 테스트 할 것!
    // cbx.find('img:visible').hide();
    // 선택요소:visible은 display:none이 아닌 요소 선택함!
