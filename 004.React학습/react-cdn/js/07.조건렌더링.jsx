...
function WishList2(props) {
    ...
    return(
        <React.Fragment>
            ...
            {
                mymv.length == 0 &&
                <h2>아직 개발자 영화 리스트가 업데이트되지 않았습니다!</h2>
            }
        </React.Fragment>
    );
} ///////// WishList2 컴포넌트 //////////////
...
function ExpComp(props) {
    ...
    // useState() 메서드가 변수값이 업데이트되는 여부를
    // 관리하여 변경 시 이 변수를 사용하는 컴포넌트를 
    // 자동으로 업데이트 한!!다!!!!!
    // 후크변수 업데이트를 set변수명(값) 형식으로 한다!

    // 변경버튼 호출 함수 //////
    const againFn = () => {
        // Not연산자 !(느낌표)는 true/false를 반대로 전환
        // [일반변수 업데이트]
        // result = !result;
        // [후크변수 업데이트]
        setResult(!result);
...
return (
    <React.Fragment>
        {/* 1. 큰 제목 */}
        <Title tit="명화" />
    {/* 2. 변경버튼 : 클릭 시 agaom함수를 호출함 */}
    <button onClick={againFn}>작가 변경</button>
        {/* 3. 작품출력 : 3항연산자로 작품 변경하기
        result변수를 후크변수로 셋팅하면 컴포넌트가
        이 변수의 값이 변경됨에 따라 자동으로
        재설정된다! */}
        {
            result ?
            <MakeWork
