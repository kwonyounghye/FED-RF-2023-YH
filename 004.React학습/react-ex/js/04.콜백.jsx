...
function MarkLike() {
    ...
    const [son, setSon] = React.useCallback(()=>{
        setSon(!son)
    },[son]);
    const [danaka, setDanaka] = React.useCallback(()=>{
        setDanaka(!danaka)
    },[danaka]);
    const [lee, setLee] = React.useCallback(()=>{
        setLee(!lee)
    },[lee]);
    ...
    **********************************************/
    ...
    return (
        <div style={{ textAlign: 'center' }}>
            <h1 style={{ fontSize: '70px', color: 'hotpink', textShadow: '5px 5px 2px #555' }}>좋아요 부탁해요!!</h1>
            {/* 좋아요 버튼 서브컴포넌트 호출 */}
...
// 메인 컴포넌트의 useCallback 처리된 함수도
// 매번 새로 그려짐! 그래서 효과가 없다!
// 따라서 호출되는 서브 컴포넌트를 메모이제이션처리해야
// useCallback도 효과를 보게 된다!!! ->>> 중요!!!!!!!!!!!!
