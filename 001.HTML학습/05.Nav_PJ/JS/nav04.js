...
function showSub() {
    ...
    if(isSub) {/* .smenu가 있는 경우만 들어옴! */
        ...
        this.style.height = (isSub.clientHeight == 0 ? hv : 0) + 'px';/* 단위 써야함 */
        ...
        gnbList.forEach(ele => {
            ...
            // 결과처리: false일 때만 높이값 0처리
            // false일 때 true처리하려면 논리부정(!)사용
            // 처리대상변수 : 순회하며 li 하위 .smenu 담기
            let target = domFn.qsEl(ele, '.smenu');

            // 현재 노드와 같은 li는 처리안함!
            if(!result) { // false일 때만 들어옴!
                // 하위에 .smenu가 없으면 처리안함!
