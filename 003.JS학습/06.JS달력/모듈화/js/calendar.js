...
/**************************************************
    ...
    this 키워드로 생성자함수 맴버등록하여 사용함!
    -> 인스턴스 코딩구역에서 생성된 인스턴스를
    변수에 담아 하위 속성 / 메서드로 호출이 가능해짐!

    4. 유의사항: 생성자함수 내부에서 this 키워드로
    등록된 속성 / 메서드는 내부에서 호출 시에도 반드시
    this 키워드를 사용하여 호출해야함!(안쓰면 에러!!)
**************************************************/ 
// DOM 메서드 //////
const dFn = {
...
function makeDallyeok(selEl) {
    ...
    let hcode = "";
    // (8) 날짜 정보 저장 히든 필드
    const dateInfo = dFn.qs(selEl+' .data-info')
    ...
    const initDallyeok = () => {
        ...
        newDate.forEach((ele) => {
            dFn.addEvt(ele, "click", () => {
                ...
                // 히든 필드에 날짜 정보 넣기 : 날짜 정보 공개
                // 활용도를 높이기 위해 일반구분자로 정보 공개
    ...
    // 3. 이벤트 설정하기 //////////////
    // 이전버튼에 함수 연결하기 :  달을 빼기 위해 -1 전달
    dFn.addEvt(dFn.qs(selEl+" .btnL"), "click", 
    () => this.chgCalendar(-1));
    // 다음버튼에 함수연결하기 : 달을 더하기 위해 1 전달
...
function insertHcode() {
    ...
    <div class="calender">
        ...
        <header class="header">      
          ...
          <div class="title">
            <div class="yearTit">2023</div>
            <div class="monthTit">11</div>
          </div>
          <!-- 달력이동버튼:다음 -->
