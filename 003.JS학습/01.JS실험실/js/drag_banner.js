...
function slideFn(selEl) { // selEl 선택 슬라이드 부모 요소
    ...
    function goSlide(){
        ...
        if(isRight){ // 오른쪽버튼
            ...
            // 이동 후엔 rx=0으로 초기화해야 버튼 클릭 시 정상 작동함!
            console.log('rx 보정값: ', rx);
...
// (5) 위치이동 차이 결과 변수 : result x -> 슬라이드와 공유
let rx = 0;
...
//   let lx = dFn.qs('.slide').offsetLeft,
    ly = 0;
  ...
  dFn.addEvt(ele, "mouseup", () => {
    dFalse();
    // lastPoint(); -> 슬라이드 드래그는 마지막 위치 업데이트 불필요!
    // 왜? 자유 드래그와 달리 슬라이드는 마지막에 항상 특정 위치에 
    // 가 있기 때문, 그리고 중간에 업데이트를 하면ㅅ 슬라이드가 튐!
...
function goWhere(target) {
    ...
    // 3-2. 오른쪽 방향 이동(왼쪽버튼 클릭과 동일)
    else if(tgLeft < -pointLeft + 50) {
        console.log('오른쪽 방향으로~!');
        // 왼쪽 버튼 클릭 이벤트 발생하기
        // 상대적으로현재위치에서 찾음
