...
// 0. 스크롤바 트랙을 잡고 위치 이동 시 위치값 반영
dFn.addEvt(window,'mouseup',()=>setPos(window.scrollY)); 
///////// mouseup ///////////////////////////

// 0. 키보드 방향키 이동 시 위치값 반영
...
function moveEl(elPos, ele, setH) {
    ...
    if(elPos < winH && elPos > -200) {
        // 1. 위치이동값 계산
        // 실제이동값 = 정한 범위 - (위치값 * 정한범위 / 전체범위)

        let x = elPos * setH / winH;
        console.log('x: ',-x);

        // 2. 해당요소의 위치값 이동 CSS 반영하기
        // Y축 이동 시 윗쪽방향은 마이너스임! -x
    ...
    /*********************************************************
        ...
        3. 정한 범위 : 이동할 수치
        4. 실제 이동값 : transform:translateY(이동수치 px)
        ___________________________________________________

        ((비례식으로 실제 이동값 알아내기))
        전체범위 : 위치값 = 정한 범위 : 실제이동값
        실제이동값 = 위치값 * 정한범위 / 전체범위

        -> 그,런,데......
        Y축 위치이동은 처음에 0부터 서서히 커지므로
        이동수치값은 정한 범위에서 실제이동값을 빼야 함!
