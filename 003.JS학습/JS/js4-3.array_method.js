...
// 1. 요구사항 : 배열에 과일 정보를 담아서 '과일주세요.' 버튼 클릭 시
// 과일 이미지 요소를 화면에 출력해준다!
// 배열 구성을 수정, 삭제 등 배열 메서드로 변경할 수 있게 한다!

// 2. 대상선정 :
// 2-1. 이벤트 대상 : .mbtn(기능 버튼들)
const mbtn = dFn.qsa(".mbtn");
// 2-2. 변경 대상 : .showit(배열 정보 출력)/.cont(과일 출력 박스)
const showit = dFn.qs(".showit");
const cont = dFn.qs(".cont");
// 2-3. 전체 과일 콤보 박스 : #sel
const sel = dFn.qs("#sel");
// 2-4. 선택 과일 콤보 박스(anum=array number):#anum
...
// join() : 전체 배열을 사이구분자를 넣고 문자열 출력

// 처음 배열 출력 함수 최초 호출!
showArray();

// 3-2. 전체 과일 콤보박스 바인딩
// 대상 : #sel/데이터 : frObj[] 배열 변수
// option 태그 변수

/////////////////////////////////////////////////////////////
// [객체의 속성(키값)을 배열로 변환하여 배열 메서드 이용하기]
// 객체 형식 -> {키:값}
...
// 갱신 시 계속 재바인딩 해야하므로 함수로 만든다!!
const bindCombo = () => {
    anum.innerHTML =
    fruit.map((v,i)=>`<option value="${i}>${v}</option>`).join('');
}; /////////// bindCombo 함수 //////////////////

// 선택 과일 콤보 바인딩함수 최초 호출!
bindCombo();
