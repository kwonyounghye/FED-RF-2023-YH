...
function localSFn() {
    ...
    // -> 객체를 생성하여 로컬 스토리지에 넣기
        ...
        // 1. 로컬쓰에 'minfo'가 없거나 'minfo'값이 '[]'배열 초기값이면 makeObj() 호출!
...
function insData() {
    ...
    // 2. 만약 하나라도 비어있다면 돌아가!
    // trim() 앞뒤 공백 제거 -> 스페이스바만 쳐도 불통과!
    if (tit.trim() == "" || cont.trim() == "") {
        alert("입력 데이터가 없습니다. 모두 입력하세요.");
        return;
    } ////////////// if ///////////////
    ...
    // 3-3. 자동 증가 번호 처리하기
    // 배열을 오름차순으로 정렬하여 맨끝 배열 데이터의 idx값을
    // 읽어온 후 숫자화하여 +1 처리함!
    // 3-3-1. 배열 오름차순 처리 :
    // -> 배열.sort((a,b)=>{
...
function delRec(idx) {
    ...
    // 3. 특정 데이터 배열 항목 삭제
    // splice(순번,개수) -> 특정 순번부터 몇 개 지움
    // 여기서는 1개 삭제이므로 splice(순번,1)
    // confirm(메시지)
    // -> 확인, 취소 중 확인 클릭 시 true 리턴함!(취소는 false)
...
// 1. 로컬쓰 가져오기
    ...
    // 3. 선택배열값인 객체의 제목과 내용을 다시 넣고 업데이트하기
    orgData[selIdx].tit = modTit.value;
    orgData[selIdx].cont = modCont.value;
