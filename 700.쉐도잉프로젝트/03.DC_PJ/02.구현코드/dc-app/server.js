...
// 서버 역할을 위한 익스프레스 생성자 메서드 불러오기(import)
const app = express();
...
// -> 배포시 이 폴더가 생기도록 SPA 개발 폴더에 public 아래에
// uploads 폴더를 만들어준다
...
// -> 첫번째값은 루트 아래에 업로드 관련 post전송을 선택
// -> upload는 폴더명이 아니고 작업명이다!!
// -> 두번째 항목은 전송종류를 설정 : 파일전송은 'file'
// -> 세번째는 내부전달 변수인 요철, 응답에 대한 함수
app.post(
...
// 서버 루트 폴더 정적 연결하기(루트 정하기)
// -> SPA에서 빌드하면 배포용 소스가 build 폴더에 
// 생성되므로 이 배포용 폴더를 Root로 잡으면 편하다!!
app.use(express.static(path.join(__dirname,'/build')));
// -> SPA 앱 빌드시 유의사항 : package.json파일에 
// home: 'http://localhost:8080'을 등록하여 사용
// localhost는 127.0.0.1 내부호출 아이피와 동일함!

// 첫페이지 설정하기 -> url로 쳐서 들어가는 경로를 설정함!
// -> get방식으로 연결하기 때문에 get() 메서드 사용!
app.get('/', function(request, response) {
    // 내부로 전달되는 값은 처음 것이 요청, 두번째가 응답임!
