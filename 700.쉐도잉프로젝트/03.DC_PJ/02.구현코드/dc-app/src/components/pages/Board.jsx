...
export function Board() {
    ...
    const bindList = () => {
        ...
        // jsn - 데이터가 변경되는 것에 따라 반영하여 상태관리함
    ...
    const pagingLink = () => {
        ...
        // 전체 레코드수 / 페이지 단위수(나머지가 있으면 +1)
        // 전체 레코드수 : pgBlock변수에 할당!
        // 2. 블록 나머지수
        const blockPad = totNum % pgBlock;

        // 최종 한계수 -> 여분 레코드 존재에 따라 1더하기
        const limit = blockCnt + (blockPad === 0 ? 0 : 1);

        // console.log("블록 개수: ", blockCnt, "\n블록 나머지: ", blockPad, "\n최종 한계수: ", limit);
    ...
    const chgList = (e) => {
        ...
        // 현재 페이지 번호 업데이트 -> 리스트 업데이트됨!
        setPgNum(currNum);
        // 바인드 리스트 호출 불필요!! 왜?
        // pgNum을 bindList()에서 사용하기 때문에
        // 리랜더링이 자동으로 일어남!!
    ...
    const chgMode = (e) => {
        ...
        if (modeTxt === "R") {
            ...
            console.log("읽기 처리", cidx);

            // 2. 해당정보 가져오기 : orgData에서 조회함
            // 전역 참조변수에 저장하여 리랜더링 시 리턴코드에
            // 이 값이 적용되게 해준다!!
            cData.current = orgData.find((v) => {
                if (v.idx === cidx) return true;
            });

            console.log("현재 data", cData.current);
        ...
        // 3-4. 글쓰기 서브밋 /////////
            ...
            const contVal = $(".writeone .content");
            ...
            // 2. 통과시 실제 데이터 입력하기
                ...
                // 3. 입력 idx 기본키값을 숫자값 중 최대값에 1을 더함!
                // 3-1. idx값만 모아서 배열로 재구성함(숫자형 변환!)
                let arrIdx = orgTemp.map(v=>v.idx);
                // 최대값
                let maxNum = Math.max(...arrIdx);
                console.log('idx배열: ',arrIdx);
                console.log('최대값: ',Math.max(arrIdx));
                console.log('다른방법 최대값: ', Math.max.apply(null,arrIdx));
                // 스프레드 연산자 나오기 전에는 항상 apply메서드 사용함
                // apply(this 객체,배열값) -> this객체 전달 없으므로 null 씀
                // -> 배열값 내부의 값을 하나씩 전달함!
