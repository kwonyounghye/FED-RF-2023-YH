...
export function Board() {
    ...
    // 파일 저장변수 업데이트함수 -> AttachBox 컴포넌트 속성으로 내려보냄!
        ...
        for (let i = initNum; i < limitNum; i++) {
            ...
            pgCode[i] = (
                <Fragment key={i}>
                    ...
                    {
                        /* 바 출력 조건 : 
        페이징의 체이징에서 끝번호 전번호일 때와
        동시에 전체한계값이 전체페이지 끝 이전번호보다 작을 때 */
    ...
    const chgMode = (e) => {
        ...
        // 3-4. 글쓰기 서브밋 /////////
            ...
            // 2. 통과시 실제 데이터 입력하기
                ...
                // 입력된 업로드 파일 정보
                console.log('업로드 파일 정보: ', uploadFile.current);
                ...
                let temp = {
                    ...
                    att: uploadFile.current ? uploadFile.current.name : "", // 파일이름 업데이트
                ...
                // [선택파일 서버 전송]
                if(uploadFile.current) { // null 아닌 할당상태일 때만

                  // 원래는 form태그로 싸여 있어서 서버 전송을 하지만
  ...
  *************************************/
    ...
    // 검색을 실행하고 다른 페이지로 이동할 수 경우
    // 데이터가 검색된 것으로 남아있으므로
    // 소멸자로 원본데이터 초기화 셋팅 함수를
    // 호출해준다!!
    useEffect(() => {
                    ...
                    <table className="dtblview writeone">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>Attachment</td>
                                <td>
                                  <a href={""+cData.current.att} download={cData.current.att}></a>
                                </td>
                                <td>
                                    <input type="text" className="subject" size="60" />
                                </td>
...
const AttachBox = ({saveFile}) => { // saveFile 프롭스펑션다운!
    ...
    // 값 : true이면 활성화, false면 비활성화
    const [isOn, setIsOn] = useState(false);
    // 2. 업로드파일 정보 관리변수
    const [uploadedInfo, setUploadedInfo] = useState(null);

    // [이벤트 처리 메서드]
    // 드래그 대상영역을 들어가고 나갈 때 isOn 상태값 업데이트하기
    const controlDragEnter = () => setIsOn(true);
    const controlDragLeave = () => setIsOn(false);
    // 드래그를 할 때 dragOver이벤트는 비활성화함!(필요가 없어서!)
